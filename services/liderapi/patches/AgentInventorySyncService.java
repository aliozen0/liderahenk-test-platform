...
public class AgentInventorySyncService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AgentInventorySyncService.class);
    ...
    public void syncAgentsFromLdap() {
        try {
            ...
            for (LdapEntry ldapAgent : ldapAgents) {
                ...
                boolean online = agentPresenceService.isOnline(jid);
    ...
    private <T> boolean setIfChanged(T currentValue, T desiredValue, java.util.function.Consumer<T> setter) {
        if (java.util.Objects.equals(currentValue, desiredValue)) {
            return false;
        }
